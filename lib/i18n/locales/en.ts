export const en = {
  "agent.description":
    "Ask one question and let Moana find the context, tools, and next steps.",
  "agent.empty": "Your agent workspace is ready.",
  "agent.input": "Ask Moana anything...",
  "agent.private": "Private by default",
  "agent.starter.image": "Generate an image",
  "agent.starter.surprise": "Surprise me",
  "agent.starter.video": "Create a video",
  "agent.starter.world": "Learn about the world",
  "agent.title": "Follow your curiosity",
  "api.compatDescription":
    "Use the same OpenAI-compatible request shape with your own client.",
  "api.compatibility": "OpenAI compatible",
  "api.connection.apiKey": "New API key",
  "api.connection.apiKeyPlaceholder": "sk-...",
  "api.connection.baseUrl": "New API base URL",
  "api.connection.baseUrlPlaceholder": "https://your-new-api.example.com",
  "api.connection.clear": "Clear connection",
  "api.connection.cleared": "Connection cleared.",
  "api.connection.clearFailed": "Failed to clear the connection.",
  "api.connection.configured": "Configured",
  "api.connection.current": "Connected to {{baseUrl}}",
  "api.connection.defaultModel": "Default model",
  "api.connection.description":
    "Use your account's New API connection for chat, agents, and media tools.",
  "api.connection.discoveryFailed": "Model discovery failed.",
  "api.connection.fetchingModels": "Fetching models...",
  "api.connection.getModels": "Get models",
  "api.connection.invalid": "Enter both the New API base URL and API key.",
  "api.connection.loadFailed": "Unable to load your connection settings.",
  "api.connection.loading": "Loading connection settings...",
  "api.connection.modelCount": "{{count}} models available",
  "api.connection.modelSelectionSaved": "Model selection saved.",
  "api.connection.modelsEnabled": "Enabled models",
  "api.connection.modelsEnabledCount": "{{count}} models selected",
  "api.connection.modelsFetched": "{{count}} models fetched.",
  "api.connection.noModels": "No models found.",
  "api.connection.save": "Save connection",
  "api.connection.saved": "Connection saved.",
  "api.connection.saveFailed": "Failed to save the connection.",
  "api.connection.saveModelSelection": "Save model selection",
  "api.connection.saveModelSelectionFailed":
    "Failed to save the model selection.",
  "api.connection.saving": "Saving...",
  "api.connection.savingModelSelection": "Saving model selection...",
  "api.connection.searchModels": "Search models...",
  "api.connection.selectAtLeastOneModel": "Select at least one model.",
  "api.connection.signInRequired":
    "Sign in to bind a New API connection to your account.",
  "api.connection.title": "Connection settings",
  "api.connection.unconfigured": "No connection configured",
  "api.connection.verifyFailed":
    "The connection was saved, but model discovery failed.",
  "api.connection.verifying": "Verifying connection...",
  "api.description":
    "Build with text, image, audio, and video models through one gateway.",
  "api.docs": "Documentation",
  "api.docsDescription":
    "Keep endpoint details and request examples close to the product.",
  "api.keyDescription":
    "Connect a New API token without exposing it in the browser.",
  "api.keys": "API keys",
  "api.modelName": "your-model",
  "api.quickstart": "Quick start",
  "api.title": "Moana API",
  "api.tryChat": "Try chat",
  "characters.browse": "Browse public characters",
  "characters.create": "Create a character",
  "characters.description":
    "Create private AI characters with their own personality and context.",
  "characters.empty": "No characters yet",
  "characters.emptyDescription":
    "Create a character to keep a distinct voice for your conversations.",
  "characters.mine": "My characters",
  "characters.name": "Name",
  "characters.namePlaceholder": "A name people will recognize",
  "characters.noDescription": "No description yet",
  "characters.personality": "Personality and context",
  "characters.personalityPlaceholder":
    "What should this character know and how should it respond?",
  "characters.publicDescription":
    "Explore characters created by the community.",
  "characters.publicTitle": "Public characters",
  "characters.save": "Save character",
  "characters.search": "Search characters",
  "characters.startChat": "Start chat",
  "chat.artifact.addComments": "Add comments",
  "chat.artifact.addedSuggestions": "Added suggestions to",
  "chat.artifact.addFinalPolish": "Add final polish",
  "chat.artifact.addingSuggestions": "Adding suggestions",
  "chat.artifact.addLogs": "Add logs",
  "chat.artifact.analyzeData": "Analyze and visualize data",
  "chat.artifact.copy": "Copy to clipboard",
  "chat.artifact.copyCode": "Copy code to clipboard",
  "chat.artifact.copyCsv": "Copy as .csv",
  "chat.artifact.copyCsvSuccess": "Copied csv to clipboard!",
  "chat.artifact.copyImage": "Copy image to clipboard",
  "chat.artifact.copyImageSuccess": "Copied image to clipboard!",
  "chat.artifact.created": "Created",
  "chat.artifact.creating": "Creating",
  "chat.artifact.errorAction": "Failed to execute action",
  "chat.artifact.errorCreate": "Error creating document:",
  "chat.artifact.errorUpdate": "Error updating document:",
  "chat.artifact.executeCode": "Execute code",
  "chat.artifact.fixError": "Fix error",
  "chat.artifact.forDocument": "for document",
  "chat.artifact.formatData": "Format and clean data",
  "chat.artifact.latest": "Latest",
  "chat.artifact.requestSuggestions": "Request suggestions",
  "chat.artifact.restore": "Restore",
  "chat.artifact.sharedUnsupported":
    "Viewing files in shared chats is currently not supported.",
  "chat.artifact.showChanges": "Show changes",
  "chat.artifact.updated": "Updated",
  "chat.artifact.updating": "Updating",
  "chat.artifact.version": "{{current}} of {{total}}",
  "chat.artifact.viewChanges": "View changes",
  "chat.artifact.viewNext": "View Next version",
  "chat.artifact.viewPrevious": "View Previous version",
  "chat.attachment.file": "File",
  "chat.auth.accountCreated": "Account created!",
  "chat.auth.accountExists": "Account already exists!",
  "chat.auth.back": "Back",
  "chat.auth.checking": "Checking authentication status, please try again!",
  "chat.auth.continueGoogle": "Continue with Google",
  "chat.auth.email": "Email",
  "chat.auth.emailPlaceholder": "you@someo.ne",
  "chat.auth.failedCreate": "Failed to create account!",
  "chat.auth.failedValidation": "Failed validating your submission!",
  "chat.auth.guest": "Guest",
  "chat.auth.googleCancelled": "Google sign-in was cancelled.",
  "chat.auth.googleFailed": "Google sign-in failed. Please try again.",
  "chat.auth.googleUnconfigured":
    "Google sign-in is not configured on this server yet.",
  "chat.auth.haveAccount": "Have an account?",
  "chat.auth.hidePassword": "Hide password",
  "chat.auth.invalidCredentials": "Invalid credentials!",
  "chat.auth.loading": "Loading",
  "chat.auth.login": "Login to your account",
  "chat.auth.loginDescription": "Welcome back! Please sign in to continue",
  "chat.auth.loginTitle": "Sign in to Moana",
  "chat.auth.noAccount": "No account?",
  "chat.auth.orDivider": "or",
  "chat.auth.password": "Password",
  "chat.auth.passwordPlaceholder": "Enter your password",
  "chat.auth.poweredBy": "Powered by",
  "chat.auth.privateByDefault": "Private by default",
  "chat.auth.registerDescription":
    "Welcome! Please fill in the details to get started",
  "chat.auth.registerTitle": "Create a Moana account",
  "chat.auth.signIn": "Sign in",
  "chat.auth.signOut": "Sign out",
  "chat.auth.signUp": "Sign up",
  "chat.auth.signUpGoogle": "Sign up with Google",
  "chat.auth.showPassword": "Show password",
  "chat.auth.submit": "Submit form",
  "chat.auth.tryWithoutAccount": "Or try Moana without an account",
  "chat.brand": "Moana",
  "chat.cancel": "Cancel",
  "chat.commands.clear": "Clear current chat",
  "chat.commands.delete": "Delete current chat",
  "chat.commands.heading": "Commands",
  "chat.commands.model": "Change the AI model",
  "chat.commands.new": "Start a new chat",
  "chat.commands.purge": "Delete all chats",
  "chat.commands.rename": "Rename current chat",
  "chat.commands.theme": "Toggle dark/light mode",
  "chat.composer.anonymous": "Ask anonymously...",
  "chat.composer.ask": "Ask anything...",
  "chat.composer.available": "Available",
  "chat.composer.edit": "Edit your message...",
  "chat.composer.editing": "Editing message",
  "chat.composer.filesTooLarge": "All files exceed the maximum size.",
  "chat.composer.modelUnavailable": "This model is not available in the demo.",
  "chat.composer.noMatchingFiles": "No files match the accepted types.",
  "chat.composer.searchModels": "Search models...",
  "chat.composer.stop": "Stop",
  "chat.composer.stopVoice": "Stop voice input",
  "chat.composer.submit": "Submit",
  "chat.composer.supportsReasoning": "Supports reasoning",
  "chat.composer.supportsTools": "Supports tool use",
  "chat.composer.supportsVision": "Supports vision",
  "chat.composer.tooManyFiles": "Too many files. Some were not added.",
  "chat.composer.uploadFiles": "Upload files",
  "chat.composer.voice": "Voice input",
  "chat.composer.voiceFailed": "Voice input failed",
  "chat.composer.voiceUnavailable": "Voice input is not supported here",
  "chat.console.clear": "Clear console",
  "chat.console.initializing": "Initializing...",
  "chat.console.label": "Console",
  "chat.console.resize": "Resize console",
  "chat.continue": "Continue",
  "chat.delete": "Delete",
  "chat.delete.description":
    "This action cannot be undone. This will permanently delete your chat and remove it from our servers.",
  "chat.delete.success": "Chat deleted",
  "chat.delete.title": "Are you absolutely sure?",
  "chat.deleteAll": "Delete all",
  "chat.deleteAll.description":
    "This action cannot be undone. This will permanently delete all your chats and remove them from our servers.",
  "chat.deleteAll.success": "All chats deleted",
  "chat.deleteAll.title": "Delete all chats?",
  "chat.folder.cancel": "Cancel folder edit",
  "chat.folder.confirm": "Save folder",
  "chat.folder.delete": "Delete folder",
  "chat.folder.delete.description":
    "The folder will be removed, but its chats will stay in your chat list.",
  "chat.folder.delete.success": "Folder deleted",
  "chat.folder.delete.title": "Delete this folder?",
  "chat.folder.dropTarget": "Drop chats into folder",
  "chat.folder.empty": "No chats in this folder",
  "chat.folder.move.success": "Chat moved to folder",
  "chat.folder.name": "Folder name",
  "chat.folder.rename": "Rename folder",
  "chat.folder.toggle": "Toggle folder",
  "chat.folder.unfile.success": "Chat removed from folder",
  "chat.folder.unfiledDropTarget": "Drop chats here to remove folder",
  "chat.folder.unfiled": "Unfiled chats",
  "chat.download": "Download conversation",
  "chat.empty.description": "Start a conversation to see messages here",
  "chat.empty.title": "No messages yet",
  "chat.gateway.activate": "Activate",
  "chat.gateway.description":
    "This application requires {{actor}} to activate Vercel AI Gateway.",
  "chat.gateway.owner": "the owner",
  "chat.gateway.title": "Activate AI Gateway",
  "chat.gateway.you": "you",
  "chat.greeting.description": "Ask a question, write code, or explore ideas.",
  "chat.greeting.title": "What can I help with?",
  "chat.header.agent": "Try Agentic Chat",
  "chat.header.chatMode": "Chat mode",
  "chat.header.chatModeNormal": "Normal chat",
  "chat.header.chatModeNormalDescription": "Chats are saved to your history.",
  "chat.header.chatModeTemporary": "Temporary chat",
  "chat.header.chatModeTemporaryDescription":
    "This chat disappears when you close the app.",
  "chat.header.newChat": "New chat",
  "chat.header.settings": "Settings",
  "chat.header.tools": "Tools",
  "chat.history": "History",
  "chat.history.empty":
    "Your conversations will appear here once you start chatting!",
  "chat.history.guest": "Login to save and revisit previous chats!",
  "chat.history.lastMonth": "Last 30 days",
  "chat.history.lastWeek": "Last 7 days",
  "chat.history.loading": "Loading...",
  "chat.history.older": "Older",
  "chat.history.today": "Today",
  "chat.history.yesterday": "Yesterday",
  "chat.image.generating": "Generating Image...",
  "chat.message.allow": "Allow",
  "chat.message.branchVersion": "{{current}} of {{total}}",
  "chat.message.copied": "Copied to clipboard!",
  "chat.message.copy": "Copy",
  "chat.message.deny": "Deny",
  "chat.message.downvote": "Downvote Response",
  "chat.message.downvoted": "Downvoted Response!",
  "chat.message.downvoteFailed": "Failed to downvote response.",
  "chat.message.downvoting": "Downvoting Response...",
  "chat.message.edit": "Edit",
  "chat.message.nextBranch": "Next branch",
  "chat.message.noText": "There's no text to copy!",
  "chat.message.previousBranch": "Previous branch",
  "chat.message.upvote": "Upvote Response",
  "chat.message.upvoted": "Upvoted Response!",
  "chat.message.upvoteFailed": "Failed to upvote response.",
  "chat.message.upvoting": "Upvoting Response...",
  "chat.message.waiting": "Waiting...",
  "chat.message.weatherDenied": "Weather lookup was denied.",
  "chat.nav.agent": "Agentic Chat",
  "chat.nav.api": "API",
  "chat.nav.characters": "Characters",
  "chat.nav.chat": "Chat",
  "chat.nav.feed": "Feed",
  "chat.nav.studio": "Studio",
  "chat.new": "New chat",
  "chat.provider.notConfigured":
    "No AI provider is configured. Open API settings to add your New API connection.",
  "chat.reasoning.fewSeconds": "Thought for a few seconds",
  "chat.reasoning.seconds": "Thought for {{duration}} seconds",
  "chat.reasoning.thinking": "Thinking...",
  "chat.rename.unavailable": "Rename is available from the sidebar chat menu.",
  "chat.scrollToBottom": "Scroll to bottom",
  "chat.settings.advanced": "Advanced settings",
  "chat.settings.disableSystemPrompt": "Disable Moana system prompt",
  "chat.settings.reasoning": "Reasoning",
  "chat.settings.reasoningDescription":
    "Allow the model to include its reasoning output when supported.",
  "chat.settings.reset": "Reset",
  "chat.settings.systemPrompt": "System prompt",
  "chat.settings.temperature": "Temperature",
  "chat.settings.title": "Settings",
  "chat.settings.topP": "Top P",
  "chat.settings.urlScraping": "URL scraping",
  "chat.settings.urlScrapingDescription":
    "Extract content from URLs in your messages.",
  "chat.settings.veniceVoice": "Moana Voice",
  "chat.settings.webSearch": "Web search",
  "chat.settings.webSearchDescription":
    "Search the web for current information.",
  "chat.sidebar.addFolder": "Add folder",
  "chat.sidebar.browseCharacters": "Browse public characters",
  "chat.sidebar.characters": "Characters",
  "chat.sidebar.charactersEmpty":
    "You haven't started any character chats yet.",
  "chat.sidebar.chats": "Chats",
  "chat.sidebar.createCharacter": "Create your own character",
  "chat.sidebar.folders": "Folders",
  "chat.sidebar.invite": "Invite and earn $10",
  "chat.sidebar.more": "More",
  "chat.sidebar.newFolder": "New folder",
  "chat.sidebar.open": "Open sidebar",
  "chat.sidebar.search": "Search",
  "chat.suggestion.apply": "Apply",
  "chat.suggestion.dismiss": "Dismiss",
  "chat.suggestion.label": "Suggestion",
  "chat.suggestions.dijkstra": "Write code to demonstrate Dijkstra's algorithm",
  "chat.suggestions.essay": "Help me write an essay about Silicon Valley",
  "chat.suggestions.nextjs": "What are the advantages of using Next.js?",
  "chat.suggestions.weather": "What is the weather in San Francisco?",
  "chat.theme.toggleDark": "Toggle dark mode",
  "chat.theme.toggleLight": "Toggle light mode",
  "chat.tool.awaitingApproval": "Awaiting Approval",
  "chat.tool.completed": "Completed",
  "chat.tool.denied": "Denied",
  "chat.tool.error": "Error",
  "chat.tool.parameters": "Parameters",
  "chat.tool.pending": "Pending",
  "chat.tool.responded": "Responded",
  "chat.tool.result": "Result",
  "chat.tool.running": "Running",
  "chat.toolbar.college": "College",
  "chat.toolbar.elementary": "Elementary",
  "chat.toolbar.graduate": "Graduate",
  "chat.toolbar.highSchool": "High School",
  "chat.toolbar.keepCurrentLevel": "Keep current level",
  "chat.toolbar.middleSchool": "Middle School",
  "chat.tools.addMedia": "Add image or document",
  "chat.tools.combineImages": "Combine images",
  "chat.tools.editImage": "Edit image",
  "chat.tools.generateAudio": "Generate audio",
  "chat.tools.imageToVideo": "Create video from image",
  "chat.tools.removeBackground": "Remove background",
  "chat.tools.styleReference": "Style reference",
  "chat.tools.summarizeVideo": "Summarize video",
  "chat.tools.title": "Tools",
  "chat.tools.upscale": "Upscale image",
  "chat.upload.failedFile": "Failed to upload file, please try again!",
  "chat.upload.failedFiles": "Failed to upload files",
  "chat.upload.failedPastedImages": "Failed to upload pasted image(s)",
  "chat.upload.pastedImage": "Pasted image",
  "chat.visibility.private": "Private",
  "chat.visibility.privateDescription": "Only you can access this chat",
  "chat.visibility.public": "Public",
  "chat.visibility.publicDescription":
    "Anyone with the link can access this chat",
  "chat.visibility.share": "Share",
  "chat.wait.response": "Please wait for the model to finish its response!",
  "chat.weather.high": "H",
  "chat.weather.hourlyForecast": "Hourly Forecast",
  "chat.weather.low": "L",
  "chat.weather.now": "Now",
  "chat.weather.sunrise": "Sunrise",
  "chat.weather.sunset": "Sunset",
  "feed.description": "Browse images and ideas shared by the Moana community.",
  "feed.empty": "No public creations yet",
  "feed.emptyNotifications": "No notifications yet",
  "feed.images": "Images",
  "feed.like": "Like",
  "feed.notifications": "Notifications",
  "feed.recent": "Recent",
  "feed.save": "Save",
  "feed.search": "Search feed",
  "feed.videos": "Videos",
  "language.chinese": "Chinese",
  "language.english": "English",
  "language.switcher": "Language",
  "search.description": "Find a saved conversation by its title.",
  "search.empty": "No matching chats",
  "search.input": "Search chats...",
  "studio.aspect": "Aspect ratio",
  "studio.audioPlaceholder": "Describe the audio you want to make...",
  "studio.createProject": "Create project",
  "studio.defaultPrompt": "Your generation is ready to start.",
  "studio.emptyDescription": "Your generated media will appear here.",
  "studio.emptyTitle": "Nothing here yet",
  "studio.generate": "Generate",
  "studio.imagePlaceholder": "Describe the image you want to make...",
  "studio.model": "Model",
  "studio.privacy": "Your drafts stay private until you choose to share them.",
  "studio.prompt": "Prompt",
  "studio.promptImage": "Describe your image",
  "studio.ready": "Ready to generate",
  "studio.settings": "Settings",
  "studio.uploadAsset": "Upload an image to begin",
  "studio.uploadHint": "PNG or JPEG up to 5 MB",
  "studio.uploadProject": "Upload project",
} as const;

export type TranslationKey = keyof typeof en;
